...
impl User {
    ...
    pub async fn find_id(pool: &SqlitePool, username: &str) -> anyhow::Result<i64> {
    ...
    pub async fn find_by_id(pool: &SqlitePool, user_id: i64) -> anyhow::Result<User> {
    ...
    pub async fn find_by_email(pool: &SqlitePool, email: &str) -> anyhow::Result<User> {
        let user = sqlx::query_as::<_, User>(
    ...
    pub async fn find_by_username(pool: &SqlitePool, username: &str) -> anyhow::Result<User> {
        let user = sqlx::query_as::<_, User>(
    ...
    pub async fn find_all(pool: &SqlitePool) -> anyhow::Result<Vec<User>> {
    ...
    pub async fn verify(pool: &SqlitePool, login: LoginRequest) -> anyhow::Result<User> {
    ...
    pub async fn exist(&self, pool: &SqlitePool) -> anyhow::Result<bool> {
    ...
    pub async fn delete_by_email(pool: &SqlitePool, email: String) -> anyhow::Result<u64> {
    ...
    pub async fn delete_by_username(pool: &SqlitePool, username: &str) -> anyhow::Result<u64> {
